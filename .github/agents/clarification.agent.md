...
2. For each question, provide **concrete options** whenever the answer space is bounded (enum values, existing keywords, yes/no, "keep current behavior vs change"). Read the code with `read`/`search` to ground the options in what actually exists (real enum members, real settings defaults, real schema columns).
