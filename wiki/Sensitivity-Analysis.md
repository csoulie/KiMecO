...
The derivative step is direction-dependent and set by the parameter's **class**. For multiplicative parameters (`if`, `sfc`, `mrc`, `bfc`, frequencies) the step is multiplicative: with factor `f = 1 + (uc - 1) * sensi_d` (where `uc` is the parameter uncertainty), the up step is `value * f` and the down step is `value / f` (e.g. `uc = 1.1`, `sensi_d = 0.1` → perturbed values `[value / 1.01, value * 1.01]`). Additive and percentage parameters use a symmetric step, `value + scale * sensi_d * side`.
